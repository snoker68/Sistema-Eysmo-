...
class TelaCadastro:
    def __init__(self, master, cadastro):
        ...
        self.conjuge = tk.Entry(self.campos_frame)
        self.data_nascimento_conjuge = tk.Entry(self.campos_frame)
        self.paroquia = tk.Entry(self.campos_frame)
        self.numero_inscrição = tk.Entry(self.campos_frame)
    ...
    def voltar_lista(self):
        TelaCadastro(self.master, self.cadastro)
        self.frame.destroy()
    ...
    def mostrar_campos(self):
        ...
        tk.Label(self.campos_frame, text="Conjuge", font=("Arial", 12)).grid(row=4, column=0, pady=2, padx=10)
        tk.Label(self.campos_frame, text="Data de Nascimento Conjuge", font=("Arial", 12)).grid(row=5, column=0, pady=2, padx=10)
        tk.Label(self.campos_frame, text="Paroquia", font=("Arial", 12)).grid(row=6, column=0, pady=2, padx=10)
        tk.Label(self.campos_frame, text="Número de Inscrição", font=("Arial", 12)).grid(row=7, column=0, pady=2, padx=10)
        ...
        self.conjuge.grid(row=4, column=1, pady=2, padx=10)
        self.data_nascimento_conjuge.grid(row=5, column=1, pady=2, padx=10)
        self.paroquia.grid(row=6, column=1, pady=2, padx=10)
        self.numero_inscrição.grid(row=7, column=1, pady=2, padx=10)

        tk.Button(self.campos_frame, text="Salvar", command=self.salvar_cadastro).grid(row=8, column=0, columnspan=1, pady=10)
        tk.Button(self.campos_frame, text="Voltar", command=self.voltar_lista).grid(row=8, column=0, columnspan=2, pady=10)
    ...
    def salvar_cadastro(self):
        ...
        conjuge = self.conjuge.get().strip()
        data_nascimento_conjuge = self.data_nascimento_conjuge.get().strip()
        paroquia = self.paroquia.get().strip()
        numero_inscrição = self.numero_inscrição.get().strip()

        if not all([nome, data_nascimento, telefone, endereco, conjuge, data_nascimento_conjuge, paroquia, numero_inscrição]):
        ...
        self.cadastro.adicionar_registro(nome, data_nascimento, telefone, endereco, conjuge, data_nascimento_conjuge, paroquia, numero_inscrição)
        self.atualizar_lista()
        ...
        self.data_nascimento_conjuge.delete(0, tk.END)
        self.paroquia.delete(0, tk.END)
        self.numero_inscrição.delete(0, tk.END)
...
class Cadastro:
    ...
    def adicionar_registro(self, nome, data_nascimento, telefone, endereco,conjuge, data_nascimento_conjuge, paroquia, numero_inscrição):
        registro = {
            "nome": nome,
            ...
            "paroquia": paroquia,
            "numero_inscrição": numero_inscrição
        }
